...
export class OffersService {
    ...
    async create(buyerId: string, dto: CreateOfferDto) {
        ...
        if (listing.sellerId === buyerId) throw new ForbiddenException("Cannot offer on own item");
        if (listing.status !== ListingStatus.PUBLISHED) throw new BadRequestException("Listing not active");
        ...
        if (existing) {
            throw new BadRequestException("You already have a pending offer on this item");
        }
    ...
    async accept(userId: string, offerId: string) {
        return this.prisma.$transaction(async (tx) => {
            ...
            if (offer.sellerId !== userId) throw new ForbiddenException("Only the seller can accept offers");
    ...
    async decline(userId: string, offerId: string) {
        ...
        if (offer.sellerId !== userId) throw new ForbiddenException("Only seller can decline");
