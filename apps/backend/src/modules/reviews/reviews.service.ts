...
export class ReviewsService {
  ...
  async listForListing(
    ...
    const [reviews, rollupRecord] = await this.prisma.$transaction([
      ...
      this.prisma.sellerReviewRollup.findUnique({
    ...
    const review = await this.prisma.$transaction(async (tx) => {
      const created = await tx.review.create({
        data: {
          ...
          rating: dto.rating,
          comment:
            dto.comment != null ? sanitizeText(dto.comment) : dto.comment,
  ...
  async update(
    ...
    const status = privileged
      ? (dto.status ?? moderation.status ?? existing.status)
      : (moderation.status ?? existing.status);
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      const review = await tx.review.update({
        ...
        data: {
          ...
          comment:
            dto.comment != null ? sanitizeText(dto.comment) : existing.comment,
          status,
  ...
  private async refreshRollups(
    prisma: Prisma.TransactionClient,
    sellerId: string,
  ): Promise<void> {
    const [published, pendingCount, flaggedCount, totalCount, starCounts] =
      await Promise.all([
        ...
        prisma.review.count({
          where: { recipientId: sellerId, status: ReviewStatus.PENDING },
        }),
        prisma.reviewFlag.count({
          where: { review: { recipientId: sellerId } },
        }),
  ...
  private async syncTrustSeed(
    prisma: Prisma.TransactionClient,
    sellerId: string,
  ): Promise<void> {
    const rollup = await prisma.sellerReviewRollup.findUnique({
      where: { sellerId },
    });
    const label = "review:aggregate";

    if (!rollup || rollup.publishedCount === 0) {
      await prisma.trustScoreSeed.deleteMany({
        where: { userId: sellerId, label },
      });
    ...
    const existingSeed = await prisma.trustScoreSeed.findFirst({
      where: { userId: sellerId, label },
    });

    if (existingSeed) {
      await prisma.trustScoreSeed.update({
        where: { id: existingSeed.id },
        data: { value },
      });
    } else {
      await prisma.trustScoreSeed.create({
        data: {
          userId: sellerId,
          label,
          value,
  ...
  private async recalculateTrustScore(
    ...
    const aggregate = await prisma.trustScoreSeed.aggregate({
      where: { userId },
      _sum: { value: true },
    });
