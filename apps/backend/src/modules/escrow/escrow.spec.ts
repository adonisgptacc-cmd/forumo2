  ...
  get escrowHolding() {
    ...
    return {
      findUnique: async ({ where, include }: any) => {
        const h = self.escrowHoldings.get(where.orderId) ?? self.escrowHoldingsById.get(where.id);
        if (!h) return null;
        return {
          ...h,
      ...
      update: async ({ where, data }: any) => {
        const h =
          self.escrowHoldings.get(where.orderId) ?? self.escrowHoldingsById.get(where.id);
        if (!h) return null;
  ...
  get escrowDispute() {
    ...
    return {
      findUnique: async ({ where, include }: any) => {
        ...
        return {
          ...d,
          escrow: self.escrowHoldingsById.get(d.escrowId),
      ...
      create: async ({ data, include }: any) => {
        const d = {
          ...
          ...data,
          openedAt: new Date(),
          openedBy: { id: data.openedById, email: 'user@test.com', name: 'User' },
          messages: [],
        };
...
describe('EscrowModule', () => {
  ...
  describe('PATCH /escrow/disputes/:disputeId/resolve', () => {
    it('resolves a dispute with REFUND action', async () => {
      ...
      expect(prismaMock.escrowDisputes.get(DISPUTE_ID)!.status).toBe('RESOLVED');
    });
