...
describe('PaymentsController — webhook integration', () => {
  ...
  beforeEach(async () => {
    ...
    jest.spyOn(paystackService, 'verifyTransaction').mockResolvedValue({
      success: true,
  ...
  describe('Stripe webhooks (POST /orders/payments/stripe/webhook)', () => {
    ...
    // The controller maps payment_intent.payment_failed → OrderStatus.REFUNDED.
    // The OrderStatus enum has no PAYMENT_FAILED variant; REFUNDED is the
    // correct terminal status for a failed payment.
    it('payment_intent.payment_failed transitions the order to REFUNDED', async () => {
      const payload = {
      ...
      expect(db.getOrderStatus(STRIPE_ORDER_2)).toBe(OrderStatus.REFUNDED);
    });
...
class WebhookInMemoryPrismaService {
  ...
  private seed() {
    ...
    this.usersMap.set(BUYER_ID, { id: BUYER_ID, email: 'buyer@test.com', name: 'Test Buyer', deletedAt: null });
    this.usersMap.set(SELLER_ID, { id: SELLER_ID, email: 'seller@test.com', name: 'Test Seller', deletedAt: null });
  ...
  private seedOrder(id: string, orderNumber: string, status: OrderStatus, now: Date) {
  ...
  private makeTimelineEvent(orderId: string, status: OrderStatus, note: string): OrderTimelineRecord {
    return { id: randomUUID(), orderId, status, note, actorId: null, metadata: null, createdAt: new Date() };
  }
  ...
  order = {
    ...
    create: async ({ data }: { data: any }) => {
      ...
      if (data.timeline?.create) {
        ...
        this.timelinesMap.set(id, events.map((e: any) => this.makeTimelineEvent(id, e.status, e.note ?? '')));
      }
      return rec;
    },
    ...
    update: async ({ where, data, include }: { where: { id: string }; data: any; include?: Prisma.OrderInclude }) => {
      ...
      if (data.timeline?.create) {
        ...
        const events = Array.isArray(data.timeline.create) ? data.timeline.create : [data.timeline.create];
        events.forEach((e: any) => existing.push(this.makeTimelineEvent(rec.id, e.status, e.note ?? '')));
  ...
  paymentTransaction = {
    ...
    updateMany: async ({ where, data }: { where: { orderId: string }; data: any }) => {
      ...
      records.forEach((r) => {
        ...
        if (data.providerStatus !== undefined) r.providerStatus = data.providerStatus;
      ...
      return (
        all.find((p) => {
          ...
          if (where.providerRef !== undefined) {
            ...
            } else if (where.providerRef !== null && typeof where.providerRef === 'object' && 'not' in where.providerRef) {
  ...
  escrowHolding = {
    ...
    findUnique: async ({ where }: { where: { orderId?: string; id?: string } }) => {
      ...
      return Array.from(this.escrowsMap.values()).find((e) => e.id === where.id) ?? null;
    },

    update: async ({ where, data }: { where: { id: string }; data: any }) => {
      const rec = Array.from(this.escrowsMap.values()).find((e) => e.id === where.id);
  ...
  payout = {
    findUnique: async ({ where, include }: { where: any; include?: any }) => {
      ...
      if (include?.seller) {
        ...
        return { ...rec, seller: { email: seller?.email ?? '', name: seller?.name ?? '' } };
      }
    ...
    update: async ({ where, data }: { where: { id: string }; data: any }) => {
      ...
      if (data.failureReason !== undefined) rec.failureReason = data.failureReason;
      if (data.scheduledAt !== undefined) rec.scheduledAt = data.scheduledAt;
      if ('paystackTransferCode' in data) rec.paystackTransferCode = data.paystackTransferCode;
      if ('stripeTransferId' in data) rec.stripeTransferId = data.stripeTransferId;

      if (data.retryCount !== undefined) {
        if (typeof data.retryCount === 'object' && 'increment' in data.retryCount) {
  ...
  private getItems(orderId: string) {
    return Array.from(this.itemsMap.values()).filter((i) => i.orderId === orderId);
  }
  ...
  private getEscrow(orderId: string) {
    ...
    return { ...escrow, disputes: [], transactions: [...(this.escrowTxnsMap.get(escrow.id) ?? [])] };
  }
}

// ─── Record types ─────────────────────────────────────────────────────────────

type UserRecord = { id: string; email: string; name: string; deletedAt: Date | null };
...
type OrderRecord = Pick<
  Order,
  | 'id' | 'orderNumber' | 'buyerId' | 'sellerId' | 'status' | 'paymentStatus'
  | 'totalItemCents' | 'shippingCents' | 'feeCents' | 'currency'
  | 'shippingAddressId' | 'billingAddressId' | 'metadata'
  | 'placedAt' | 'paidAt' | 'fulfilledAt' | 'deliveredAt' | 'cancelledAt'
  | 'createdAt' | 'updatedAt'
>;

type OrderItemRecord = {
  id: string; orderId: string; listingId: string; listingTitle: string;
  variantId: string | null; variantLabel: string | null;
  quantity: number; unitPriceCents: number; currency: string;
};

type OrderTimelineRecord = {
  id: string; orderId: string; status: OrderStatus;
  note: string | null; actorId: string | null;
  metadata: Prisma.JsonValue | null; createdAt: Date;
};

type PaymentTransactionRecord = {
  id: string; orderId: string; provider: PaymentProvider;
  status: PaymentStatus; providerStatus: string | null;
  amountCents: number; currency: string; providerRef: string | null;
  metadata: Prisma.JsonValue | null; processedAt: Date | null;
  createdAt: Date; updatedAt: Date;
};

type EscrowHoldingRecord = {
  id: string; orderId: string; status: EscrowStatus;
  amountCents: number; currency: string;
  releaseAfter: Date | null; releasedAt: Date | null;
  metadata: Prisma.JsonValue | null;
};

type EscrowTransactionRecord = {
  id: string; escrowId: string; type: EscrowTransactionType;
  amountCents: number; currency: string;
...
type PayoutRecord = {
  ...
  status: PayoutStatus; paystackTransferCode: string | null;
  stripeTransferId: string | null; retryCount: number;
