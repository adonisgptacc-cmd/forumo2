...
function toShipmentStatus(shippoStatus: string): ShipmentStatus {
  switch (shippoStatus.toUpperCase()) {
    case 'TRANSIT':    return ShipmentStatus.IN_TRANSIT;
    case 'DELIVERED':  return ShipmentStatus.DELIVERED;
    case 'RETURNED':   return ShipmentStatus.RETURNED;
    case 'FAILURE':    return ShipmentStatus.CANCELLED;
    default:           return ShipmentStatus.IN_TRANSIT;
  }
}
...
export class ShippoWebhookController {
  ...
  async handleShippoWebhook(
    ...
    const { tracking_number: trackingNumber, carrier, tracking_status, tracking_history, eta } = data;
    ...
    const shipment = await this.prisma.orderShipment.findFirst({
      ...
      include: { order: { include: { escrow: true, buyer: { select: { id: true, email: true, name: true } } } } },
    });

    if (!shipment) {
      this.logger.warn(`Shippo webhook: no shipment found for tracking ${trackingNumber} (${carrier})`);
      return { received: true };
    }
    ...
    await this.prisma.orderShipment.update({
      ...
      data: {
        ...
        ...(currentStatus.toUpperCase() === SHIPPO_DELIVERED_STATUS && { deliveredAt: new Date() }),
      },
    });

    this.logger.log(`Shippo tracking update: order=${orderId} status=${currentStatus}`);
  ...
  private async handleDelivered(
    ...
    if (order.escrow && order.escrow.status === EscrowStatus.HOLDING && !order.escrow.releaseAfter) {
    ...
    if (order.buyer) {
      ...
      void this.notifications.createInApp(order.buyer.id, 'order.delivered', {
        orderId,
  ...
  private verifySignature(req: Request, signature?: string): void {
    ...
    if (!secret) return; // verification skipped when no secret is configured
