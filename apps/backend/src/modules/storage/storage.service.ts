...
export class StorageService {
  ...
  async saveListingImage(
    ...
    const key = `listings/${listingId}/${Date.now()}-${randomUUID()}-${this.sanitizeFilename(file.originalname)}`;
  ...
  async saveMessageAttachment(
    ...
    const key = `messages/${threadId}/${Date.now()}-${randomUUID()}-${this.sanitizeFilename(file.originalname)}`;
    return this.persistFile(key, file);
  }
  ...
  async saveKycDocument(
    ...
    const key = `kyc/${userId}/${Date.now()}-${randomUUID()}-${this.sanitizeFilename(file.originalname)}`;
    return this.persistFile(key, file);
  }
