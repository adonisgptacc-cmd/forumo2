...
export class OtpDeliveryService {
  ...
  constructor(private readonly configService: ConfigService) {
    if (this.hasMailgunConfig()) {
      this.mailgunConfig = {
        ...
        from:
          this.configService.get<string>("MAILGUN_EMAIL_FROM") ??
          "no-reply@forumo.dev",
        apiBase:
          this.configService.get<string>("MAILGUN_API_BASE") ??
  ...
  private async sendEmail(
    ...
    if (this.mailgunConfig) {
      ...
      try {
        const response = await fetch(
          `${this.mailgunConfig.apiBase}/v3/${this.mailgunConfig.domain}/messages`,
          {
            method: "POST",
            headers: {
        ...
        if (!response.ok) {
          ...
          this.logger.warn(
            `Mailgun delivery failed: ${message || response.statusText}`,
          );
          return this.simulateDelivery(
            NotificationChannel.EMAIL,
            recipient,
            code,
          );
        ...
        this.logger.error(
          `Mailgun delivery threw: ${(error as Error).message}`,
        );
        return this.simulateDelivery(
          NotificationChannel.EMAIL,
          recipient,
    ...
    this.logger.debug(
      `Mailgun credentials missing, simulating email delivery to ${recipient}`,
    );
  ...
  private async sendSms(
    ...
    this.logger.debug(
      `SNS credentials missing, simulating SMS delivery to ${recipient}`,
    );
    return this.simulateDelivery(NotificationChannel.SMS, recipient, code);
  }
