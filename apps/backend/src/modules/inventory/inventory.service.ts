...
export class InventoryService {
    ...
    async getInventoryByVariant(variantId: string) {
        ...
        const damagedQuantity = items.reduce((sum, item) => sum + item.damagedQuantity, 0);
    ...
    async markDamaged(itemId: string, quantity: number, reason?: string) {
        ...
        if (item.availableQuantity < quantity) {
            throw new BadRequestException('Insufficient available stock to mark as damaged');
        }
        ...
        const updated = await this.prisma.inventoryItem.update({
            ...
            data: {
                ...
                metadata: ({
                    ...
                    damageReports: [
                        ...(((item.metadata as Record<string, unknown>)?.damageReports as unknown[]) || []),
                        {
                            quantity,
                            reason,
        ...
        const updated = await this.prisma.inventoryItem.update({
            ...
            data: {
                ...
                metadata: ({
                    ...((item.metadata as Record<string, unknown>) || {}),
                    adjustments: [
                        ...(((item.metadata as Record<string, unknown>)?.adjustments as unknown[]) || []),
                        {
                            adjustment,
                            reason,
