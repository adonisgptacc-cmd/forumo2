...
describe("ListingSearchService", () => {
  ...
  beforeEach(() => {
    prisma = {
      ...
      $transaction: jest
        .fn()
        .mockImplementation((actions: Promise<unknown>[]) =>
          Promise.all(actions),
        ),
    };
  ...
  it("applies relevance ranking when keyword is provided", async () => {
    ...
    expect(result.data.map((listing) => listing.id)).toEqual([
      listingB.id,
      listingA.id,
    ]);
  ...
  it("uses a fixed-length key only for bounded low-cardinality searches", async () => {
    const cache = {
    ...
    expect(cache.get).toHaveBeenCalledWith(
      expect.stringMatching(/^listings:search:[a-f0-9]{64}$/),
    );
    ...
    await service.search({ keyword: "woven", page: 1, pageSize: 20 });

    expect(cache.get).not.toHaveBeenCalled();
    expect(cache.set).not.toHaveBeenCalled();
  });

  it("rejects oversized search inputs before querying or caching", async () => {
    const cache = {
      get: jest.fn().mockResolvedValue(undefined),
      set: jest.fn().mockResolvedValue(undefined),
    };
    ...
    await expect(
      service.search({ keyword: "x".repeat(201), page: 1, pageSize: 20 }),
    ).rejects.toThrow("keyword must be 200 characters or fewer");
  ...
  it("normalizes keyword spacing and uses multiple tsquery strategies", () => {
    ...
    expect(tsQueries.baseQuery.strings.join(" ")).toContain(
