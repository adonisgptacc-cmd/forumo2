...
export class AuctionsService {
    ...
    async create(userId: string, dto: CreateAuctionDto) {
        ...
        if (listing.sellerId !== userId) throw new ForbiddenException('Not owner of listing');
        if (listing.status === ListingStatus.PUBLISHED) throw new BadRequestException('Listing already published');
    ...
    async findOne(id: string) {
        const auction = await this.prisma.auction.findUnique({
            ...
            include: {
                listing: { include: { images: true } },
                bids: { orderBy: { amountCents: 'desc' }, take: 10, include: { bidder: { select: { id: true, name: true, avatarUrl: true } } } },
                seller: { select: { id: true, name: true, avatarUrl: true } },
            },
        });
    ...
    async placeBid(userId: string, auctionId: string, dto: PlaceBidDto) {
        const createdBid = await this.prisma.$transaction(async (tx) => {
            ...
            if (!auction) throw new NotFoundException('Auction not found');
            if (auction.sellerId === userId) throw new ForbiddenException('Cannot bid on own auction');
            if (auction.status !== AuctionStatus.ACTIVE) throw new BadRequestException('Auction not active');
            if (new Date() > auction.endAt) throw new BadRequestException('Auction ended');
            ...
            const currentDisplayPrice = highestBid ? highestBid.amountCents : auction.startingBidCents;
            const minIncrement = this.getMinIncrement(currentDisplayPrice);
            const minBidRequired = highestBid ? currentDisplayPrice + minIncrement : auction.startingBidCents;

            // The user's maximum bid
            const newMaxBid = dto.maxAutoBidCents || dto.amountCents;

            if (newMaxBid < minBidRequired) {
                throw new BadRequestException(`Bid must be at least ${(minBidRequired / 100).toFixed(2)}`);
            }
            ...
            if (!highestBid) {
                ...
                const bid = await tx.bid.create({
                    ...
                    include: { bidder: { select: { id: true, name: true, avatarUrl: true } } }
                });
            ...
            const currentMaxBid = highestBid.maxAutoBidCents || highestBid.amountCents;

            if (userId === highestBid.bidderId) {
                // Same user increasing their max bid
                if (newMaxBid <= currentMaxBid) {
                    throw new BadRequestException('New max bid must be higher than current max bid');
                }
