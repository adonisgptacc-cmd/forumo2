        ...
        for (const auction of endedAuctions) {
            ...
            try {
                ...
                await this.prisma.$transaction(async (tx) => {
                    ...
                    // Check reserve
                    if (auction.reserveCents && winningBid.amountCents < auction.reserveCents) {
                        await tx.auction.update({
                            where: { id: auction.id },
                    ...
                    this.logger.log(`Auction ${auction.id} won by ${winningBid.bidderId}. Order ${order.id} created.`);
