...
function SellerReviewsSection({ sellerId }: { sellerId: string }) {
  const { data: rollup, isLoading: rollupLoading } =
    useSellerReviewRollup(sellerId);
  ...
  const starKey = (s: number): keyof typeof rollup =>
    `star${s}` as keyof typeof rollup;
  const pct = (n: number) =>
    rollup.publishedCount > 0
      ? Math.round((n / rollup.publishedCount) * 100)
      : 0;
    ...
    <div className="card p-6 space-y-5">
      ...
      <div className="flex items-center gap-6">
        <div className="text-center">
          <p className="text-5xl font-bold text-[color:var(--ink)]">
          ...
          {[5, 4, 3, 2, 1].map((star) => {
            ...
            return (
              <div key={star} className="flex items-center gap-2">
                ...
                <span className="text-[color:var(--ink-3)] text-xs w-8 text-right">
                  {pct(count)}%
                </span>
              </div>
            );
      ...
      {rollup.pendingCount > 0 && (
        <p className="text-xs text-[color:var(--ink-3)]">
          {rollup.pendingCount} review{rollup.pendingCount !== 1 ? "s" : ""}{" "}
          pending moderation
        </p>
      )}
    </div>
  ...
  return (
    <div className="flex items-center justify-between gap-3 bg-[color:var(--surface-2)] border-b border-[color:var(--line)] px-4 py-2.5">
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        <a href={`forumo://shop/${slug}`} className="btn btn-primary btn-sm">
          Open app
        </a>
        <button
          type="button"
      ...
      <div className="min-h-screen bg-[color:var(--bg)] flex items-center justify-center">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-[color:var(--ink-3)]">
            This shop may not exist or has been removed.
          </p>
          <Link
            href="/listings"
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-end gap-5 -mt-12 relative z-10">
          ...
          <div className="pb-2 flex-1 min-w-0">
            <h1 className="text-2xl font-bold text-[color:var(--ink)] truncate">
              {storefront.name}
            </h1>
            <p className="text-sm text-[color:var(--ink-3)]">
              @{storefront.slug}
        ...
        {storefront.description && (
          <p className="mt-4 text-sm text-[color:var(--ink-2)] max-w-2xl">
            {storefront.description}
          </p>
        )}
        ...
        <div className="mt-10">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-lg font-semibold text-[color:var(--ink)]">
              {storefront.user
                ? `All listings by ${storefront.user.name ?? storefront.name}`
                : "Listings"}
            </h2>
          </div>
          {storefront.user?.id ? (
            ...
            <div className="py-12 text-center text-[color:var(--ink-3)] text-sm">
