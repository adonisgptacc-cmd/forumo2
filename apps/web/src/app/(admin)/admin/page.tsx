...
const toneStyles: Record<string, string> = {
  ...
  slate: "text-[color:var(--ink-3)] bg-[color:var(--surface-2)] border-[color:var(--line)]",
};
...
function StatusBadge({ label }: { label: string }) {
  const palette: Record<string, string> = {
    ...
    Completed: "bg-emerald-50 text-emerald-700 border-emerald-200",
    CANCELLED: "bg-[color:var(--surface-2)] text-[color:var(--ink-3)] border-[color:var(--line)]",
    FLAGGED: "bg-red-50 text-red-700 border-red-200",
    Flagged: "bg-red-50 text-red-700 border-red-200",
    REMOVED: "bg-[color:var(--surface-2)] text-[color:var(--ink-3)] border-[color:var(--line)]",
    Removed: "bg-[color:var(--surface-2)] text-[color:var(--ink-3)] border-[color:var(--line)]",
    ...
    Refunded: "bg-sky-50 text-sky-700 border-sky-200",
    DRAFT: "bg-[color:var(--surface-2)] text-[color:var(--ink-3)] border-[color:var(--line)]",
    PAUSED: "bg-amber-50 text-amber-700 border-amber-200",
    APPROVED: "bg-emerald-50 text-emerald-700 border-emerald-200",
    REJECTED: "bg-red-50 text-red-700 border-red-200",
  };

  return <span className={`rounded-full border px-2.5 py-1 text-xs ${palette[label] ?? "bg-[color:var(--surface-2)] text-[color:var(--ink-2)] border-[color:var(--line)]"}`}>{label}</span>;
}

function LineChart({ data }: { data: { label: string; value: number }[] }) {
  if (!data || data.length < 2) {
    return <div className="h-48 card flex items-center justify-center text-sm text-[color:var(--ink-3)]">No data yet</div>;
  }
      ...
      <svg viewBox="0 0 100 100" className="h-full w-full" preserveAspectRatio="none">
        <defs>
          <linearGradient id="line" x1="0" x2="0" y1="0" y2="1">
            <stop offset="0%" stopColor="oklch(0.50 0.070 155)" stopOpacity="0.8" />
            <stop offset="100%" stopColor="oklch(0.50 0.070 155)" stopOpacity="0" />
          </linearGradient>
        </defs>
        <polyline
          points={points}
  ...
  return (
    <div className="h-48 card p-4">
      <div className="flex h-full items-end gap-3">
        {data.map((item) => (
          <div key={item.label} className="flex-1 text-center text-xs text-[color:var(--ink-3)]">
            <div
  ...
  return (
    <div className="relative h-48 w-full">
      ...
      <div className="absolute inset-0 flex items-center justify-center text-center">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-[color:var(--ink-3)]">Categories</p>
          <p className="text-2xl font-semibold text-[color:var(--ink)]">{total}%</p>
          <p className="text-xs text-[color:var(--ink-3)]">Distribution of active listings</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="text-[10px] uppercase tracking-[0.3em] text-[color:var(--ink-3)]">Marketplace overview</p>
          <h2 className="text-3xl font-semibold text-[color:var(--ink)]">Executive dashboard</h2>
          <p className="text-sm text-[color:var(--ink-3)]">
            Monitor marketplace health, revenue, and trust signals from a single control plane.
          </p>
        </div>
        <div className="flex flex-wrap gap-2 text-sm">
          <Link href="/admin/categories" className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-emerald-700 hover:bg-emerald-100 transition-colors">
            + Add category
          </Link>
          <button
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {metrics.map((metric) => (
          <div key={metric.label} className="card p-4 shadow-lg">
            <p className="text-xs uppercase tracking-[0.3em] text-[color:var(--ink-3)]">{metric.label}</p>
            <div className="mt-2 flex items-end justify-between gap-3">
              <h3 className="text-3xl font-semibold text-[color:var(--ink)]">{metric.value}</h3>
              <span className={`rounded-full border px-2 py-1 text-xs ${toneStyles[metric.tone]}`}>{metric.delta}</span>
            </div>
            <p className="mt-1 text-xs text-[color:var(--ink-3)]">Week-over-week change</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[2fr,1fr]">
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <div className="flex items-center justify-between pb-3">
              <div>
                <p className="text-[10px] uppercase tracking-[0.3em] text-[color:var(--ink-3)]">Sales</p>
                <h3 className="text-lg text-[color:var(--ink)]">Revenue trend</h3>
              </div>
              <span className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs text-emerald-700">Live</span>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between pb-3">
              <div>
                <p className="text-[10px] uppercase tracking-[0.3em] text-[color:var(--ink-3)]">Users</p>
                <h3 className="text-lg text-[color:var(--ink)]">New registrations</h3>
              </div>
              <span className="rounded-full border border-sky-200 bg-sky-50 px-3 py-1 text-xs text-sky-700">+12% MoM</span>
            </div>
        ...
        <div className="card p-4">
          <div className="flex items-center justify-between pb-3">
            <div>
              <p className="text-[10px] uppercase tracking-[0.3em] text-[color:var(--ink-3)]">Inventory mix</p>
              <h3 className="text-lg text-[color:var(--ink)]">Category performance</h3>
            </div>
            <button className="text-xs text-[color:var(--accent)] hover:text-[color:var(--accent-2)]">View details →</button>
          </div>
          <div className="grid gap-4 md:grid-cols-[1.1fr,0.9fr]">
            <DonutChart data={categoryPerformance} />
            <div className="space-y-3 text-sm">
              {categoryPerformance.map((item) => (
                <div key={item.label} className="flex items-center justify-between rounded-lg border border-[color:var(--line)] bg-[color:var(--surface-2)] px-3 py-2">
                  <div className="flex items-center gap-3">
                    <span className={`h-2.5 w-2.5 rounded-full ${toneDots[item.tone] ?? "bg-slate-500"}`}></span>
                    <div>
                      <p className="font-medium text-[color:var(--ink)]">{item.label}</p>
                      <p className="text-xs text-[color:var(--ink-3)]">{item.value}% of active listings</p>
                    </div>
                  </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[2fr,1fr]">
        <div className="card p-4">
          <div className="flex flex-wrap items-center justify-between gap-3 pb-4">
            <div>
              <p className="text-[10px] uppercase tracking-[0.3em] text-[color:var(--ink-3)]">Recent activity</p>
              <h3 className="text-lg text-[color:var(--ink)]">Listings, purchases, registrations</h3>
            </div>
            <button
            ...
            {recentActivity.map((activity) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-[color:var(--ink)]">{activity.title}</p>
                    <p className="text-xs text-[color:var(--ink-3)]">{activity.meta}</p>
                  </div>
                </div>
                <p className="text-xs text-[color:var(--ink-3)]">{activity.time}</p>
              </div>
            ))}
          </div>
        </div>
        <div className="space-y-4">
          <div className="card p-4">
            <div className="flex items-center justify-between pb-3">
              <div>
                <p className="text-[10px] uppercase tracking-[0.3em] text-[color:var(--ink-3)]">Quick actions</p>
                <h3 className="text-lg text-[color:var(--ink)]">Resolve workflows</h3>
              </div>
            </div>
            <div className="space-y-2">
              <a href="/admin/kyc" className="flex w-full items-center justify-between rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-700 hover:bg-emerald-100 transition-colors">
                Approve verified sellers <span className="text-xs">{stats?.pendingKyc ?? '...'} pending →</span>
              </a>
              <a href="/admin/moderations" className="flex w-full items-center justify-between rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800 hover:bg-amber-100 transition-colors">
                Review flagged listings <span className="text-xs">{stats?.pendingModeration ?? '...'} pending →</span>
              </a>
              <a href="/admin/disputes" className="flex w-full items-center justify-between rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700 hover:bg-red-100 transition-colors">
          ...
          <div className="card p-4">
            <div className="flex items-center justify-between pb-3">
              <div>
                <p className="text-[10px] uppercase tracking-[0.3em] text-[color:var(--ink-3)]">Reports</p>
                <h3 className="text-lg text-[color:var(--ink)]">Analytics & exports</h3>
              </div>
              <span className="rounded-full border border-[color:var(--line)] bg-[color:var(--surface-2)] px-2 py-1 text-[11px] text-[color:var(--ink-2)]">CSV, PDF</span>
            </div>
            <div className="space-y-3 text-sm">
              {reports.map((report) => (
                <div key={report.title} className="flex items-center justify-between rounded-lg border border-[color:var(--line)] bg-[color:var(--surface-2)] px-3 py-2">
                  <div>
                    <p className="font-medium text-[color:var(--ink)]">{report.title}</p>
                    <p className="text-xs text-[color:var(--ink-3)]">{report.detail}</p>
                  </div>
                  <button className="text-xs text-[color:var(--accent)] hover:text-[color:var(--accent-2)]">{report.action} →</button>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <div className="card p-4">
          <div className="flex flex-wrap items-center justify-between gap-3 pb-4">
            <div>
              <p className="text-[10px] uppercase tracking-[0.3em] text-[color:var(--ink-3)]">Users</p>
              <h3 className="text-lg text-[color:var(--ink)]">Account management</h3>
            </div>
            ...
            <table className="min-w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-[color:var(--line)] text-[color:var(--ink)]">
                {filteredUsers.length === 0 && (
                  <tr><td colSpan={6} className="px-3 py-6 text-center text-[color:var(--ink-3)] text-sm">No users found</td></tr>
                )}
                {filteredUsers.map((u) => (
                  <tr key={u.id} className="hover:bg-[color:var(--surface-2)]">
                    <td className="px-3 py-2">
                      <div>
                        ...
                        <p className="text-xs text-[color:var(--ink-3)]">{u.email}</p>
                      </div>
                    </td>
                    <td className="px-3 py-2 text-[color:var(--ink-2)]">{u.role}</td>
                    <td className="px-3 py-2">
                      <StatusBadge label={u.kycStatus} />
                    </td>
                    <td className="px-3 py-2 text-[color:var(--ink-2)]">{u.listingsCount}</td>
                    <td className="px-3 py-2 text-[color:var(--ink-2)]">{new Date(u.createdAt).toLocaleDateString()}</td>
        ...
        <div className="card p-4">
          <div className="flex flex-wrap items-center justify-between gap-3 pb-4">
            <div>
              <p className="text-[10px] uppercase tracking-[0.3em] text-[color:var(--ink-3)]">Listings</p>
              <h3 className="text-lg text-[color:var(--ink)]">Inventory controls</h3>
            </div>
            <div className="flex flex-wrap gap-2 text-xs text-[color:var(--ink-2)]">
              <input
                className="input-forumo w-48"
            ...
            <table className="min-w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-[color:var(--line)] text-[color:var(--ink)]">
                {filteredListings.length === 0 && (
                  <tr><td colSpan={5} className="px-3 py-6 text-center text-[color:var(--ink-3)] text-sm">No listings found</td></tr>
                )}
                {filteredListings.map((l) => (
                  <tr key={l.id} className="hover:bg-[color:var(--surface-2)]">
                    ...
                    <td className="px-3 py-2 text-[color:var(--ink-2)]">{new Date(l.createdAt).toLocaleDateString()}</td>
                    <td className="px-3 py-2 text-right text-xs text-amber-200">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <div className="card p-4">
          <div className="flex flex-wrap items-center justify-between gap-3 pb-4">
            <div>
              <p className="text-[10px] uppercase tracking-[0.3em] text-[color:var(--ink-3)]">Transactions</p>
              <h3 className="text-lg text-[color:var(--ink)]">Order history</h3>
            </div>
            <div className="flex flex-wrap gap-2 text-xs text-[color:var(--ink-2)]">
              <select
                className="input-forumo w-32"
            ...
            <table className="min-w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-[color:var(--line)] text-[color:var(--ink)]">
                {filteredTransactions.length === 0 && (
                  <tr><td colSpan={6} className="px-3 py-6 text-center text-[color:var(--ink-3)] text-sm">No orders found</td></tr>
                )}
                {filteredTransactions.map((o) => (
                  <tr key={o.id} className="hover:bg-[color:var(--surface-2)]">
                    <td className="px-3 py-2 font-medium font-mono text-xs">
                      <Link href={`/admin/orders/${o.id}` as any} className="text-[color:var(--accent)] hover:underline">
                        #{o.orderNumber}
                      </Link>
                    </td>
                    <td className="px-3 py-2 text-[color:var(--ink-2)]">{o.buyerName ?? o.buyerEmail ?? '—'}</td>
                    <td className="px-3 py-2"><StatusBadge label={o.status} /></td>
                    <td className="px-3 py-2"><StatusBadge label={o.paymentStatus} /></td>
                    <td className="px-3 py-2 text-[color:var(--ink-2)]">{fmt(o.totalItemCents, o.currency)}</td>
                    <td className="px-3 py-2 text-[color:var(--ink-2)]">{o.placedAt ? new Date(o.placedAt).toLocaleDateString() : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="card p-4">
          <div className="flex flex-wrap items-center justify-between gap-3 pb-4">
            <div>
              <p className="text-[10px] uppercase tracking-[0.3em] text-[color:var(--ink-3)]">Categories</p>
              <h3 className="text-lg text-[color:var(--ink)]">Hierarchy management</h3>
            </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {categoryPerformance.map((category) => (
              <div key={category.label} className="rounded-xl border border-[color:var(--line)] bg-[color:var(--surface-2)] p-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className={`h-2 w-2 rounded-full ${toneDots[category.tone] ?? "bg-slate-500"}`}></span>
                    <p className="font-medium text-[color:var(--ink)]">{category.label}</p>
                  </div>
                  <span className={`rounded-full border px-2 py-1 text-[11px] ${toneStyles[category.tone]}`}>{category.value}%</span>
                </div>
                <p className="mt-1 text-xs text-[color:var(--ink-3)]">Performance across {Math.round(category.value * 1200)} listings</p>
