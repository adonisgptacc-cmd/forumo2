...
const STATUS_COLORS: Record<string, string> = {
  ...
  REFUNDED:  'border-[color:var(--line)] bg-[color:var(--surface-2)] text-[color:var(--ink-3)]',
...
const SHIPMENT_COLORS: Record<string, string> = {
  ...
  PENDING:    'border-[color:var(--line)] bg-[color:var(--surface-2)] text-[color:var(--ink-3)]',
};
...
function Badge({ label, colorMap }: { label: string; colorMap: Record<string, string> }) {
  const cls = colorMap[label] ?? 'border-[color:var(--line)] bg-[color:var(--surface-2)] text-[color:var(--ink-3)]';
  return (
  ...
  if (isError || !order) {
    return (
      <div className="p-6 alert alert-error text-center rounded-xl">
        ...
        <Link href="/admin" className="text-sm text-[color:var(--accent)] hover:underline">← Back to Admin</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <Link href="/admin" className="text-xs text-[color:var(--ink-3)] hover:text-[color:var(--ink)]">← Admin dashboard</Link>
          <h1 className="mt-1 text-2xl font-bold text-[color:var(--ink)]">Order {order.orderNumber}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <section className="card p-5 space-y-1">
          ...
          <p className="text-sm font-medium text-[color:var(--ink)]">{(order as any).buyer?.name ?? order.buyerId}</p>
          <p className="text-xs text-[color:var(--ink-2)]">{(order as any).buyer?.email ?? '—'}</p>
          <p className="text-xs text-[color:var(--ink-3)]">ID: {order.buyerId}</p>
        </section>
        <section className="card p-5 space-y-1">
          <h3 className="eyebrow">Seller</h3>
          <p className="text-sm font-medium text-[color:var(--ink)]">{(order as any).seller?.name ?? order.sellerId}</p>
          <p className="text-xs text-[color:var(--ink-2)]">{(order as any).seller?.email ?? '—'}</p>
          <p className="text-xs text-[color:var(--ink-3)]">ID: {order.sellerId}</p>
        </section>
      </div>

      {/* Items */}
      <section className="card p-5 space-y-3">
        <h3 className="text-sm font-semibold text-[color:var(--ink)]">Line items</h3>
        <ul className="divide-y divide-[color:var(--line)]">
          {order.items.map((item) => (
            <li key={item.id} className="flex items-center justify-between py-3 text-sm">
              <div>
                <p className="font-medium text-[color:var(--ink)]">{item.listingTitle}</p>
                {item.variantLabel && <p className="text-xs text-[color:var(--ink-3)]">{item.variantLabel}</p>}
                <p className="text-xs text-[color:var(--ink-3)]">Qty: {item.quantity}</p>
              </div>
              <p className="text-[color:var(--ink-2)]">{fmt(item.unitPriceCents * item.quantity, item.currency)}</p>
            </li>
          ))}
        </ul>
      ...
      {order.escrow && (
        <section className="card p-5 space-y-2">
          <h3 className="text-sm font-semibold text-[color:var(--ink)]">Escrow</h3>
          <div className="flex items-center justify-between text-sm">
            <span className="text-[color:var(--ink-2)]">{fmt(order.escrow.amountCents, order.escrow.currency)}</span>
            <Badge label={order.escrow.status} colorMap={ESCROW_COLORS} />
          </div>
          {order.escrow.releaseDate && (
            <p className="text-xs text-[color:var(--ink-3)]">Release date: {new Date(order.escrow.releaseDate).toLocaleString()}</p>
          )}
          {(order.escrow as any).disputes && (order.escrow as any).disputes.length > 0 && (
            <div className="mt-2 space-y-1">
              <p className="text-xs font-semibold text-orange-700">Disputes</p>
              {((order.escrow as any).disputes as any[]).map((d: any) => (
                <div key={d.id} className="rounded-lg border border-orange-200 bg-orange-50 p-3 text-xs space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-orange-700 font-medium">{d.status}</span>
                    <span className="text-[color:var(--ink-3)]">{new Date(d.createdAt ?? d.openedAt).toLocaleDateString()}</span>
                  </div>
                  <p className="text-[color:var(--ink-2)]">{d.reason}</p>
                  {d.resolution && <p className="text-emerald-700">Resolution: {d.resolution}</p>}
                </div>
              ))}
            </div>
          )}
      ...
      {order.shipments && order.shipments.length > 0 && (
        <section className="card p-5 space-y-2">
          <h3 className="text-sm font-semibold text-[color:var(--ink)]">Shipment</h3>
            ...
            <div key={s.id} className="space-y-1 text-sm">
              ...
              {s.carrier && (
                <p className="text-[color:var(--ink-3)]">
                  Carrier: <span className="text-[color:var(--ink)]">{s.carrier}</span>
                </p>
              )}
              {s.trackingNumber && (
                <p className="text-[color:var(--ink-3)]">
                  Tracking: <span className="font-mono text-[color:var(--ink)]">{s.trackingNumber}</span>
                </p>
              )}
              {s.estimatedDelivery && (
                <p className="text-xs text-[color:var(--ink-3)]">ETA: {new Date(s.estimatedDelivery).toLocaleDateString()}</p>
              )}
            </div>
          ))}
        </section>
      ...
      {/* Timeline */}
      <section className="card p-5 space-y-3">
        <h3 className="text-sm font-semibold text-[color:var(--ink)]">Timeline</h3>
        <ol className="relative border-l border-[color:var(--line)] pl-5 space-y-4">
          {order.timeline.map((event) => (
            <li key={event.id} className="relative">
              ...
              <p className="text-sm font-medium text-[color:var(--ink)]">{event.status}</p>
              ...
              {(event as any).actor && (
                <p className="text-xs text-[color:var(--ink-3)]">by {(event as any).actor.name ?? (event as any).actor.email}</p>
              )}
            </li>
          ))}
        </ol>
      ...
      {/* Admin actions */}
      <section className="card p-5 space-y-3">
        <h3 className="text-sm font-semibold text-[color:var(--ink)]">Admin actions</h3>
        <p className="text-xs text-[color:var(--ink-3)]">Force-update order status (use with caution).</p>
