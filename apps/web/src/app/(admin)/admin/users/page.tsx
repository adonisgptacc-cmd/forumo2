...
const ROLE_COLORS: Record<string, string> = {
  ...
  BUYER: 'bg-[color:var(--surface-2)] text-[color:var(--ink-3)] border-[color:var(--line)]',
};
...
function UserRow({ user }: { user: AdminUserDetail }) {
  const roleCls = ROLE_COLORS[user.role] ?? 'bg-[color:var(--surface-2)] text-[color:var(--ink-3)] border-[color:var(--line)]';
...
export default function AdminUsersPage() {
  ...
  if (isError || !users) {
    return (
      <p className="text-sm text-red-600">Failed to load users. Make sure you have admin privileges.</p>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-[color:var(--ink)]">Users</h2>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-[color:var(--line)] bg-[color:var(--surface-2)]">
              <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-[color:var(--ink-3)]">User</th>
              <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-[color:var(--ink-3)]">Role</th>
              <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-[color:var(--ink-3)]">KYC</th>
              <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-[color:var(--ink-3)]">Listings</th>
              <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-[color:var(--ink-3)]">Joined</th>
            </tr>
          </thead>
          <tbody>
            {users.map((user) => (
