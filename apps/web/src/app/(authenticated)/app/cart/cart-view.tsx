...
export function CartView() {
  const { items, itemCount, totalCents, groupedBySeller, removeItem, updateQuantity, clearCart } = useCart();
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Shopping Cart ({itemCount} item{itemCount !== 1 ? 's' : ''})</h1>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
          ...
          {Array.from(groupedBySeller.entries()).map(([sellerId, sellerItems]) => (
            <div key={sellerId} className="card-forumo space-y-4">
              ...
              {sellerItems.map((item) => (
                <div key={`${item.listingId}:${item.variantId ?? ''}`} className="flex gap-4 pb-4 border-b border-slate-100 last:border-0 last:pb-0">
                  {/* Image */}
                  ...
                  <div className="flex-1 min-w-0">
                    <Link href={`/listings/${item.listingId}`} className="text-sm font-medium hover:text-forumo-link line-clamp-2">
                      {item.title}
                    </Link>
                    {item.variantLabel && (
                      <p className="text-xs muted mt-0.5">{item.variantLabel}</p>
                    )}
                    <p className="text-base font-bold mt-1">{formatPrice(item.priceCents, item.currency)}</p>
                  </div>
                  {/* Quantity + Remove */}
                  <div className="flex flex-col items-end gap-2 flex-shrink-0">
                    ...
                    >Remove</button>
                    <p className="text-sm font-semibold">{formatPrice(item.priceCents * item.quantity, item.currency)}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="card-forumo h-fit space-y-3">
          ...
          <Link href="/listings" className="block text-center text-sm text-forumo-link hover:underline">
            Continue shopping
          </Link>
        </div>
      </div>
