...
const BADGE: Record<string, string> = {
  ...
  CLOSED: 'border-[color:var(--line)] text-[color:var(--ink-3)] bg-[color:var(--surface-2)]',
};
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm muted">Orders under escrow dispute review</p>
        </div>
          ...
          {tabs.map(({ key, label }) => (
            <button
              ...
              className={`rounded-md px-4 py-1.5 text-sm transition ${
                tab === key ? 'bg-[color:var(--ink)] text-[color:var(--bg)]' : 'muted hover:text-[color:var(--ink)]'
              }`}
            >
              {label}
            </button>
      ...
      {!isLoading && filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-[color:var(--line-2)] py-20 text-center">
          ...
          <p className="text-lg font-semibold text-[color:var(--ink)]">No disputes — great!</p>
          <p className="mt-1 text-sm muted">All your orders are running smoothly.</p>
          <Link
        ...
        <div className="rounded-xl border border-[color:var(--line)]">
                  ...
                  return (
                    ...
                    >
                      <td className="px-5 py-4">
                        <span className="font-mono text-xs text-[color:var(--ink-2)]">{order.orderNumber}</span>
                      </td>
          ...
          <div className="sm:hidden divide-y divide-[color:var(--line)]">
            {filtered.map((order) => {
              ...
              return (
                <div key={order.id} className="px-4 py-4 space-y-2 bg-[color:var(--surface)]">
                  ...
                  {firstItem && (
                    <p className="text-sm text-[color:var(--ink)] truncate">{firstItem.listingTitle}</p>
                  )}
