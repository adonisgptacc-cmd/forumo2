...
export function AuctionForm() {
  ...
  const publishedListings = (data?.data ?? []).filter(
    (l: SafeListing) => l.status === "PUBLISHED",
  );
  const selected = publishedListings.find(
    (l: SafeListing) => l.id === selectedId,
  );
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      ...(reservePrice
        ? { reserveCents: Math.round(parseFloat(reservePrice) * 100) }
        : {}),
      ...(buyNowPrice
        ? { buyNowCents: Math.round(parseFloat(buyNowPrice) * 100) }
        : {}),
    };
    ...
    return (
      <div className="py-10 text-center space-y-3">
        <p className="text-[color:var(--ink-3)]">
          You have no published listings to auction.
        </p>
        <p className="text-sm text-[color:var(--ink-3)]">
          Publish a listing first, then come back to create an auction.
        </p>
        <a href="/app/listings/new" className="btn btn-primary inline-flex">
          Create a listing
        </a>
      </div>
    );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        {selected && (
          <p className="mt-1 text-xs text-[color:var(--escrow)]">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          />
          <p className="mt-1 text-xs text-[color:var(--ink-3)]">
            Minimum you&apos;ll accept. Not shown to buyers.
          </p>
        </div>
        <div>
      ...
      {selected && startingBid && (
        <div className="rounded-xl border border-[color:var(--line)] bg-[color:var(--surface-2)] p-4 space-y-1 text-sm">
          <p className="font-medium text-[color:var(--ink-2)]">
            Auction summary
          </p>
          <p className="text-[color:var(--ink-3)]">
            Item:{" "}
            <span className="text-[color:var(--ink)]">{selected.title}</span>
          </p>
          <p className="text-[color:var(--ink-3)]">
            Starting bid:{" "}
            <span className="text-[color:var(--accent)]">
              {parseFloat(startingBid) > 0
                ? parseFloat(startingBid).toLocaleString("en-GH", {
                    style: "currency",
          ...
          <p className="text-[color:var(--ink-3)]">
            ...
            <span className="text-[color:var(--ink)]">
              {durationDays} day{parseInt(durationDays) > 1 ? "s" : ""}
            </span>
          </p>
          {buyNowPrice && (
            <p className="text-[color:var(--ink-3)]">
              Buy now:{" "}
              <span className="text-[color:var(--escrow)]">
                {parseFloat(buyNowPrice).toLocaleString("en-GH", {
                  style: "currency",
                  currency: selected.currency ?? "GHS",
                })}
              </span>
      ...
      {createAuction.isError && (
        <p className="rounded-lg border border-red-200 bg-red-50 px-4 py-2 text-sm text-red-700">
          {(createAuction.error as Error)?.message ??
    ...
    >
      <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-md bg-[color:var(--surface-2)]">
        {listing.images?.[0]?.url ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-[color:var(--ink-3)] text-sm">
            📦
          </div>
        )}
      </div>
