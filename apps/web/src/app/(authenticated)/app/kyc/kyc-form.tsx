  ...
  if (status === 'unverified') {
    return (
      <div className="flex items-center gap-3 rounded-xl border border-[color:var(--line-2)] bg-[color:var(--surface-2)]/60 px-5 py-4">
        <svg className="w-5 h-5 text-[color:var(--ink-3)] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
        </svg>
        <p className="text-sm text-[color:var(--ink-2)]">Verify your identity to start selling</p>
      </div>
    );
  }
  ...
  if (status === 'pending') {
    return (
      <div className="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-5 py-4">
        <svg className="w-5 h-5 text-[color:var(--accent)] flex-shrink-0 animate-spin" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
        </svg>
        <p className="text-sm text-amber-800">Under review — usually 1–3 business days</p>
      </div>
    );
  }
  ...
  if (status === 'approved') {
    return (
      <div className="flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-5 py-4">
        <svg className="w-5 h-5 text-[color:var(--escrow)] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
        </svg>
        <p className="text-sm text-emerald-700 font-medium">Identity verified</p>
      </div>
    );
  }
      ...
      <div className="flex">
        {labels.map((label, i) => (
          <div key={i} className="flex-1 text-center">
            <span className={`text-xs ${i < step ? 'text-[color:var(--accent)]' : 'text-[color:var(--ink-3)]'}`}>
              {label}
            </span>
          </div>
        ))}
...
function FileDropZone({
      ...
      <div className="flex items-center gap-4 rounded-xl border border-[color:var(--line-2)] bg-[color:var(--surface-2)] p-4">
        {preview ? (
          ...
          <div className="w-16 h-16 rounded-lg bg-[color:var(--surface-2)] flex items-center justify-center flex-shrink-0">
            <svg className="w-7 h-7 text-[color:var(--ink-3)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
            </svg>
          </div>
        )}
        <div className="flex-1 min-w-0">
          <p className="text-sm text-[color:var(--ink)] truncate">{file.name}</p>
          <p className="text-xs text-[color:var(--ink-3)] mt-0.5">{(file.size / 1024 / 1024).toFixed(2)} MB</p>
        </div>
        <button
          onClick={onClear}
  ...
  return (
    <div>
      ...
      >
        <svg className="w-8 h-8 text-[color:var(--ink-3)] mx-auto mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5m-13.5-9L12 3m0 0 4.5 4.5M12 3v13.5" />
        </svg>
        <p className="text-sm text-[color:var(--ink-2)]">{label}</p>
        {hint && <p className="text-xs text-[color:var(--ink-3)] mt-1">{hint}</p>}
        ...
        <section className="rounded-xl card p-6 space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-semibold text-[color:var(--ink)]">Identity Verification</h3>
              <span className="text-xs text-[color:var(--ink-3)]">Step {step} of 5</span>
            </div>
          ...
          {isDirty && (
            <div className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-2.5">
              <svg className="w-4 h-4 text-[color:var(--accent)] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
              </svg>
              <p className="text-xs text-amber-800">Your progress will be lost if you navigate away</p>
            </div>
          )}
                ...
                {DOC_TYPES.map((dt) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <span className="text-sm font-medium text-[color:var(--ink)]">{dt.label}</span>
                    </div>
                    <ul className="space-y-0.5 pl-6">
                      {dt.requirements.map((r) => (
                        <li key={r} className="text-xs text-[color:var(--ink-3)]">· {r}</li>
                      ))}
                    </ul>
                  </button>
                ))}
            ...
            <div className="space-y-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-[color:var(--ink-3)]">Make sure all text is clearly visible</p>
                </div>
          ...
          {step === 4 && (
            <div className="space-y-4">
              <div>
                <h4 className="text-sm font-medium text-[color:var(--ink)] mb-2">Selfie with your document</h4>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-4 rounded-lg border border-[color:var(--line)] bg-[color:var(--surface-2)] px-4 py-3">
                  <span className="text-xs text-[color:var(--ink-3)] w-28 flex-shrink-0">Document type</span>
                ...
                {(
                  ...
                  return (
                    ...
                    >
                      <span className="text-xs text-[color:var(--ink-3)] w-28 flex-shrink-0">{label}</span>
                      <span className="text-sm text-[color:var(--ink)] truncate flex-1">{file.name}</span>
