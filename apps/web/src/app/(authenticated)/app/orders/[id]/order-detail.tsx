...
export function OrderDetail({ id }: { id: string }) {
  ...
  const { data: escrowDetails } = useEscrowDetails(
    order?.status === "DISPUTED" ? id : null,
  );
  ...
  const statusColor =
    ...
    "text-[color:var(--ink-3)] border-[color:var(--line)]";
  ...
  const canRequestRefund =
    isBuyer &&
    order.escrow?.status === "HOLDING" &&
    !["CANCELLED", "REFUNDED", "DISPUTED", "COMPLETED", "PENDING"].includes(
      order.status,
    );
  ...
  const needsPayment =
    ...
    (!order.payments ||
      ...
      order.payments.every(
        (p) => p.status !== "SETTLED" && p.status !== "CAPTURED",
      ));
    ...
    <div className="space-y-6">
      {stripeRedirectStatus === "succeeded" && (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 flex items-center gap-3 fade-up">
          ...
          <div>
            <p className="font-semibold text-[color:var(--escrow)]">
              Payment confirmed
            </p>
            <p className="text-sm muted">
              Your payment was processed successfully. The order will update
              shortly.
            </p>
          </div>
        </div>
      ...
      {stripeRedirectStatus === "failed" && (
        <div className="rounded-xl border border-red-200 bg-red-50 p-4 fade-up">
          ...
          <p className="text-sm muted">
            Your card was not charged. Please try again from your order page.
          </p>
        </div>
      )}
      {/* Header */}
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <span
          className={`rounded-full border px-4 py-1 text-sm font-medium ${statusColor}`}
        >
          {order.status}
        </span>
      </div>
      ...
      <section className="card card-pad space-y-3">
        ...
        <div className="border-t border-[color:var(--line)] pt-3 space-y-1 text-sm">
          <div className="flex justify-between muted">
            ...
            <span>
              {(order.totalItemCents / 100).toFixed(2)} {order.currency}
            </span>
          </div>
          {order.shippingCents > 0 && (
            <div className="flex justify-between muted">
              <span>Shipping</span>
              <span>
                {(order.shippingCents / 100).toFixed(2)} {order.currency}
              </span>
            </div>
          )}
          {order.feeCents > 0 && (
            <div className="flex justify-between muted">
              ...
              <span>
                {(order.feeCents / 100).toFixed(2)} {order.currency}
              </span>
            </div>
          )}
          <div className="flex justify-between font-semibold text-[color:var(--ink)] pt-1">
            <span>Total</span>
            <span>
              {(totalCents / 100).toFixed(2)} {order.currency}
            </span>
          </div>
        </div>
      </section>
        ...
        <section className="card card-pad space-y-2">
          ...
          <div className="flex items-center justify-between">
            <span className="text-sm muted">
              {(order.escrow.amountCents / 100).toFixed(2)}{" "}
              {order.escrow.currency}
            </span>
            <span
              className={`rounded-full border px-3 py-0.5 text-xs font-medium ${ESCROW_COLORS[order.escrow.status] ?? "text-[color:var(--ink-3)] border-[color:var(--line)]"}`}
            >
              {order.escrow.status}
            </span>
          </div>
          ...
          return (
            <section className="rounded-xl border border-orange-200 bg-orange-50 p-5 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs rounded-full border border-orange-300 px-2 py-0.5 text-orange-700">
                  {dispute.status}
                </span>
              </div>
              <p className="text-xs muted">
                Reason: <span className="subtle">{dispute.reason}</span>
              </p>
              ...
              <div className="space-y-2">
                {(dispute.messages ?? []).length === 0 && (
                  <p className="text-xs muted">
                    No messages yet. Add a message below to communicate with the
                    admin.
                  </p>
                )}
              ...
              {dispute.status === "RESOLVED" && (
                <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 space-y-1">
                  <p className="text-sm font-semibold text-[color:var(--escrow)]">
                    ✅ Dispute resolved
                  </p>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={async () => {
                      ...
                      await addDisputeMessage.mutateAsync({
      ...
      {(order.status === "FULFILLED" ||
        ...
        <section className="card card-pad space-y-3">
          ...
          {isBuyer && (!order.shipments || order.shipments.length === 0) && (
            <p className="text-xs muted">
              The seller hasn&apos;t added tracking info yet.
            </p>
          )}
            ...
            <div className="space-y-3 rounded-lg border border-[color:var(--line)] bg-[color:var(--surface-2)] p-4">
              ...
              {(createShipment.isError || updateShipment.isError) && (
                <p className="text-xs text-red-600">
                  {
                    ((createShipment.error ?? updateShipment.error) as Error)
                      ?.message
                  }
                </p>
              )}
          ...
          <section className="card card-pad space-y-4">
            ...
            {!labelResult && showLabelPanel && (
              <div className="space-y-4 rounded-lg border border-[color:var(--line)] bg-[color:var(--surface-2)] p-4">
                <p className="text-xs muted">
                  Enter your address and parcel info to get carrier rates from
                  Shippo.
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setLabelFromCountry(e.target.value.toUpperCase())
                      }
                      placeholder="GH"
                      maxLength={2}
                  ...
                  {[
                    {
                      label: "Weight (g)",
                      value: labelWeight,
                      setter: setLabelWeight,
                    },
                    {
                      label: "Length (cm)",
                      value: labelLength,
                      setter: setLabelLength,
                    },
                    {
                      label: "Width (cm)",
                      value: labelWidth,
                      setter: setLabelWidth,
                    },
                    {
                      label: "Height (cm)",
                      value: labelHeight,
                      setter: setLabelHeight,
                    },
                ...
                {getShippingRates.isError && (
                  <p className="text-xs text-red-600">
                    {(getShippingRates.error as Error)?.message}
                  </p>
                )}
                ...
                {shippingRates.length > 0 && (
                  <div className="space-y-2">
                      ...
                      >
                        ...
                        {rate.estimatedDays != null && (
                          <p className="text-xs muted mt-0.5">
                            ...
                            {rate.estimatedDays !== 1 ? "s" : ""} estimated
                          </p>
                        )}
                      </button>
                    ))}
                    {purchaseLabel.isError && (
                      <p className="text-xs text-red-600">
                        {(purchaseLabel.error as Error)?.message}
                      </p>
                    )}
                    <div className="flex gap-2">
                      <button
                        ...
                        onClick={async () => {
                          ...
                          const result =
                            await purchaseLabel.mutateAsync(selectedRateId);
      ...
      <section className="card card-pad space-y-4">
        ...
        {needsPayment && (
          ...
          >
            <p className="text-sm text-[color:var(--accent-2)] font-medium">
        ...
        {showDisputeForm && (
          <div className="rounded-lg border border-orange-200 bg-orange-50 p-4 space-y-3">
            ...
            <p className="text-xs muted">
              Describe the issue in detail. An admin will review and mediate
            ...
            {openDispute.isError && (
              <p className="text-xs text-red-600">
                {(openDispute.error as Error)?.message}
              </p>
            )}
            <div className="flex gap-2">
              <button
        ...
        {showRefundForm && (
          <div className="rounded-lg border border-red-200 bg-red-50 p-4 space-y-3">
            ...
            <p className="text-xs muted">
              Describe the issue. The seller will be notified and an admin may
              review the request.
            </p>
            <textarea
              value={refundReason}
