...
function StatCard({ label, value, accent = false }: { label: string; value: string | number; accent?: boolean }) {
  return (
    <div className="rounded-xl card p-4 space-y-1">
      ...
      <p className={`text-xl font-semibold ${accent ? 'text-[color:var(--accent)]' : 'text-[color:var(--ink)]'}`}>{value}</p>
    </div>
  );
}
...
function RevenueSection({ period, groupBy }: { period: AnalyticsPeriod; groupBy: AnalyticsGroupBy }) {
  ...
  if (isError || !data?.length) {
    return (
      <div className="rounded-xl card p-5 h-64 flex items-center justify-center">
        <p className="text-sm text-[color:var(--ink-3)]">No revenue data for this period.</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl card p-5 space-y-3">
      <h3 className="text-sm font-semibold text-[color:var(--ink-2)]">Revenue</h3>
      <ResponsiveContainer width="100%" height={220}>
        <LineChart data={chartData} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
          ...
          <Tooltip
            contentStyle={{ background: '#ffffff', border: '1px solid #e5e0d4', borderRadius: 8, fontSize: 12 }}
            formatter={(value) => [`$${Number(value ?? 0).toLocaleString()}`, 'Revenue']}
          />
          <Line type="monotone" dataKey="revenue" stroke="#c2683a" strokeWidth={2} dot={false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
  ...
  if (isError || !data?.length) {
    return (
      <div className="rounded-xl card p-5 h-32 flex items-center justify-center">
        <p className="text-sm text-[color:var(--ink-3)]">No listing data yet.</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl card overflow-hidden">
      <div className="px-5 py-3 border-b border-[color:var(--line)]">
        <h3 className="text-sm font-semibold text-[color:var(--ink-2)]">Top Listings</h3>
      </div>
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-[color:var(--line)]">
          {data.map((listing: SellerTopListing) => (
            <tr key={listing.listingId} className="hover:bg-[color:var(--surface-2)] transition-colors">
              <td className="px-5 py-3 text-[color:var(--ink-2)] truncate max-w-[200px]">{listing.title}</td>
              <td className="px-5 py-3 text-right font-medium text-[color:var(--accent)]">{fmt(listing.revenue)}</td>
              <td className="px-5 py-3 text-right text-[color:var(--ink-2)]">{listing.orders}</td>
              <td className="px-5 py-3 text-right text-[color:var(--ink-3)]">{listing.views ?? '—'}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <div className="rounded-xl card p-5 space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-[color:var(--ink-2)]">Reviews</h3>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-[color:var(--ink-3)]">Seller Analytics</p>
