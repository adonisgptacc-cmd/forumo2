...
test("unauthenticated visitors are redirected away from admin surfaces", async ({
  page,
}) => {
...
test("non-privileged users see an authorization error", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "You need elevated privileges" }),
  ).toBeVisible();
});

test("admins can process KYC, listing moderation, and dispute workflows", async ({
  page,
}) => {
  ...
  await page
    .getByPlaceholder("Resolution notes")
    .fill("Customer refunded after evidence review.");
