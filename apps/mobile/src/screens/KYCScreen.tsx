...
export const KYCScreen: React.FC = () => {
  ...
  const load = useCallback(async () => {
    try {
      const data = await apiClient.get<KycSubmission>("/kyc/status", {
        auth: true,
      });
      setKyc(data);
    } catch {
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              const data = await apiClient.post<KycSubmission>(
                "/kyc/submit",
                formData,
                { auth: true },
              );
              ...
              Alert.alert(
                "Submitted",
                "Your KYC has been submitted for review. We will notify you once reviewed.",
              );
            } catch (e: any) {
  ...
  const statusStyle = kyc
    ? (STATUS_STYLES[kyc.status] ?? STATUS_STYLES.PENDING)
    : null;
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.subtitle}>
        Verify your identity to unlock higher trust scores, higher selling
        limits, and escrow access.
      </Text>
