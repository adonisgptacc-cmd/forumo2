...
export const EditListingScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  const [price, setPrice] = useState(initialListing ? String(initialListing.priceCents / 100) : '');
  ...
  const [published, setPublished] = useState(initialListing?.status === 'PUBLISHED');

  // Fetch if no initial data
  useEffect(() => {
    if (initialListing) return;
    apiClient.listings.get(listingId).then((l) => {
      setTitle(l.title);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.section}>
        ...
        <TextInput style={styles.input} value={title} onChangeText={setTitle} placeholder="What are you selling?" testID="edit-title" />
        ...
        <TextInput style={styles.input} value={location} onChangeText={setLocation} placeholder="City, country" testID="edit-location" />

        <View style={styles.switchRow}>
          <View>
            <Text style={styles.label}>Published</Text>
            <Text style={styles.switchSub}>Off = draft (not visible to buyers)</Text>
          </View>
          <Switch
            value={published}
...
const styles = StyleSheet.create({
  ...
  section: { backgroundColor: brandColors.card, borderRadius: 14, padding: spacing.md, marginBottom: spacing.md, gap: 4 },
  label: { fontSize: 13, fontWeight: '600', color: brandColors.muted, marginTop: 10, marginBottom: 4 },
  required: { color: '#ef4444' },
  input: { borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 10, paddingHorizontal: 14, paddingVertical: 10, fontSize: 15, backgroundColor: '#f9fafb' },
  ...
  saveBtn: { backgroundColor: brandColors.primary, borderRadius: 14, paddingVertical: 16, alignItems: 'center', marginBottom: spacing.sm },
