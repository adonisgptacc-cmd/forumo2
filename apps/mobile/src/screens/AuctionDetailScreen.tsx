...
export const AuctionDetailScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    const connectSocket = async () => {
      try {
        ...
        // Authenticate via first message instead of query string (prevents token leakage to logs/proxies)
        if (accessToken) {
  ...
  const placeBid = async () => {
    ...
    const currentBid =
      auction?.currentBidCents ?? auction?.startingBidCents ?? 0;
    if (amountCents <= currentBid) {
      Alert.alert(
        "Bid Too Low",
        `Your bid must exceed ${formatCents(currentBid, auction?.currency)}.`,
      );
      return;
    }

    const dto: { amountCents: number; maxAutoBidCents?: number } = {
      amountCents,
    };
    ...
    try {
      ...
      Alert.alert(
        "Bid Placed",
        `Your bid of ${formatCents(amountCents, auction?.currency)} was placed.`,
      );
      await loadAuction();
    } catch (err: any) {
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            setBidding(true);
            try {
              await apiClient.auctions.placeBid(auctionId, {
                amountCents: auction.buyNowCents!,
              });
              ...
              Alert.alert(
                "Failed",
                err.message ?? "Could not complete purchase.",
              );
            } finally {
              setBidding(false);
            }
    ...
    >
      ...
      <View style={styles.liveRow}>
        <View
          style={[
            ...
            {
              backgroundColor:
                liveStatus === "connected" ? "#22c55e" : "#d1d5db",
            },
          ]}
        />
      ...
      <View style={styles.bidBox}>
        <View style={styles.bidBoxCol}>
          ...
          <Text style={styles.bidValue}>
            {formatCents(currentBid, auction?.currency)}
          </Text>
        ...
        <View style={[styles.bidBoxCol, styles.alignRight]}>
          ...
          <Text style={[styles.bidValue, isSnipingZone && styles.snipingText]}>
            {remaining || "—"}
          </Text>
          {isSnipingZone && (
            <Text style={styles.snipingBadge}>⚡ Anti-snipe active</Text>
          )}
        </View>
      </View>
      ...
      {isSnipingZone && (
        <View style={styles.snipingNotice}>
          <Text style={styles.snipingNoticeText}>
            Anti-snipe: any bid in the last 2 minutes extends the auction by 2
            minutes.
          </Text>
        </View>
      )}
      ...
      {auction?.reserveCents ? (
        <Text style={styles.meta}>
          Reserve: {formatCents(auction.reserveCents, auction.currency)}
        </Text>
      ) : null}
          ...
          bids.map((bid) => (
            <View key={bid.id} style={styles.bidRow}>
              ...
              <Text style={styles.bidAmountText}>
                {formatCents(bid.amountCents, bid.currency)}
              </Text>
            </View>
          ))
        )}
