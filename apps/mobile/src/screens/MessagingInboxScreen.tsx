...
const ThreadCard: React.FC<{
  ...
  onPress: (thread: SafeMessageThread) => void;
}> = ({ thread, onPress }) => {
  ...
  return (
    ...
    >
      ...
      {latestMessage ? (
        <Text numberOfLines={2}>{latestMessage.body}</Text>
      ) : (
...
export const MessagingInboxScreen: React.FC = () => {
  ...
  return (
    <FlatList
      ...
      renderItem={({ item }) => (
        <ThreadCard thread={item} onPress={openThread} />
      )}
      refreshControl={
        <RefreshControl refreshing={loading} onRefresh={loadThreads} />
      }
      ListEmptyComponent={
          ...
          {!user ? (
            <View style={styles.banner}>
              <Text style={styles.subtitle}>
                Sign in to reply or continue with demo threads.
              </Text>
              <Button
                title="Go to login"
