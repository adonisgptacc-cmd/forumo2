...
export const OrderDetailScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  const disputeOrder = async () => {
    Alert.alert('Open Dispute', 'Are you sure you want to open a dispute for this order?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Open Dispute',
        style: 'destructive',
  ...
  return (
    ...
    >
      ...
      <View style={styles.headerSection}>
        ...
        {order.placedAt ? (
          <Text style={styles.dateText}>Placed: {formatDate(order.placedAt)}</Text>
        ) : null}
      </View>
      ...
      <View style={styles.section}>
        ...
        {order.items.map((item) => (
          <View key={item.id} style={styles.itemRow}>
            <View style={styles.itemInfo}>
              <Text style={styles.itemTitle} numberOfLines={2}>{item.listingTitle}</Text>
        ...
        <View style={styles.priceSummary}>
          <View style={styles.priceRow}>
            ...
            <Text style={styles.priceValue}>{formatCents(order.totalItemCents, order.currency)}</Text>
          </View>
          {order.shippingCents > 0 && (
            <View style={styles.priceRow}>
              <Text style={styles.priceLabel}>Shipping</Text>
              <Text style={styles.priceValue}>{formatCents(order.shippingCents, order.currency)}</Text>
            </View>
          )}
          {order.feeCents > 0 && (
            <View style={styles.priceRow}>
              <Text style={styles.priceLabel}>Platform fee</Text>
              <Text style={styles.priceValue}>{formatCents(order.feeCents, order.currency)}</Text>
            </View>
          )}
          <View style={[styles.priceRow, styles.totalRow]}>
            <Text style={styles.totalLabel}>Total</Text>
            <Text style={styles.totalValue}>{formatCents(totalCents, order.currency)}</Text>
          </View>
        </View>
      </View>
      ...
      {/* Buyer actions */}
      {isBuyer && (order.status === 'DELIVERED' || order.status === 'FULFILLED') && (
        <View style={styles.actionsSection}>
          ...
          >
            {actionLoading ? (
              ...
              <Text style={styles.actionBtnText}>✓ Confirm Receipt & Release Payment</Text>
            )}
          </TouchableOpacity>
          <TouchableOpacity
          ...
          >
            <Text style={[styles.actionBtnText, { color: '#ef4444' }]}>⚠ Open Dispute</Text>
          </TouchableOpacity>
        </View>
      )}
        ...
        <View style={styles.actionsSection}>
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              setActionLoading(true);
              try {
                const updated = await apiClient.orders.updateStatus(orderId, { status: 'FULFILLED', note: 'Item shipped' });
                setOrder(updated);
              } catch (err: any) {
...
const styles = StyleSheet.create({
  ...
  escrowRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 4 },
  escrowLabel: { color: brandColors.muted },
  escrowValue: { fontWeight: '600' },
  timelineRow: { flexDirection: 'row', gap: 12, paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#f3f4f6' },
  ...
  actionsSection: { marginHorizontal: spacing.md, marginBottom: spacing.lg, gap: spacing.sm },
  actionBtn: { borderRadius: 12, paddingVertical: 14, alignItems: 'center' },
  releaseBtn: { backgroundColor: brandColors.primary },
  disputeBtn: { backgroundColor: '#fff', borderWidth: 1, borderColor: '#fca5a5' },
  btnDisabled: { opacity: 0.6 },
  actionBtnText: { color: '#fff', fontWeight: '700', fontSize: 15 },
  sellerNotice: { backgroundColor: '#eff6ff', borderRadius: 10, padding: spacing.md },
