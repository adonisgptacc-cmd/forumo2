...
const AuctionCard: React.FC<AuctionCardProps> = ({ item, onPress }) => {
  ...
  return (
    <TouchableOpacity style={styles.card} onPress={onPress} testID={`auction-card-${item.id}`}>
      <View style={styles.cardHeader}>
        <Text style={styles.cardTitle} numberOfLines={1}>{item.listing?.title ?? 'Auction'}</Text>
        <View style={[styles.timerBadge, isEnding && styles.timerBadgeUrgent]}>
          <Text style={[styles.timerText, isEnding && styles.timerTextUrgent]}>{remaining}</Text>
        </View>
      </View>
      <View style={styles.cardRow}>
        <View>
          ...
          <Text style={styles.currentBid}>
            {formatCents(item.currentBidCents ?? item.startingBidCents, item.currency)}
          </Text>
        ...
        {item.buyNowCents ? (
          <View style={styles.buyNowBox}>
            ...
            <Text style={styles.buyNowPrice}>{formatCents(item.buyNowCents, item.currency)}</Text>
          </View>
        ) : null}
      </View>
    </TouchableOpacity>
...
export const AuctionsTab: React.FC = () => {
  ...
  const loadPage = useCallback(
    async (pageToLoad: number, append = true) => {
      ...
      setError(undefined);
      try {
        const response = await apiClient.auctions.list({ page: pageToLoad, pageSize: 10, status: 'ACTIVE' });
        const items: Auction[] = Array.isArray(response?.data) ? response.data : DEMO_AUCTIONS;
    ...
    <View style={styles.container} testID="auctions-tab">
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <AuctionCard
            item={item}
            onPress={() => navigation.push('AuctionDetail', { auctionId: item.id, auction: item })}
          />
        )}
        ...
        ListEmptyComponent={
          !loading ? <Text style={styles.empty}>No active auctions right now.</Text> : null
        }
        ListFooterComponent={
          loading ? <View style={styles.footer}><ActivityIndicator /></View> : null
        }
      />
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  timerBadge: { backgroundColor: '#f3f4f6', borderRadius: 6, paddingHorizontal: 8, paddingVertical: 4 },
